...
-- ============================================================
-- BOTAO REDONDO CUSTOMIZADO - Coloque seu Roblox Asset ID abaixo!
-- Arraste | Clique para abrir/fechar | RightShift = atalho
-- ============================================================
do
    -- >> CONFIGURE AQUI <<
    local IMAGE_ID = "rbxassetid://127239295635967"  -- Troque pelo seu Asset ID de imagem
    local BUTTON_SIZE = 52                       -- Tamanho do botao em pixels
    local INITIAL_POS = UDim2.new(0, 15, 0, 15) -- Posicao inicial (X, Y)
    ...
    ScreenGui.Name = "SaturnRoundBtn"
    ScreenGui.ResetOnSpawn = false
    ScreenGui.DisplayOrder = 999
    ScreenGui.IgnoreGuiInset = true
    ScreenGui.Parent = CoreGui

    -- Sombra (frame levemente maior atras para dar profundidade)
    local Shadow = Instance.new("ImageLabel")
    Shadow.Name = "Shadow"
    Shadow.Size = UDim2.new(0, BUTTON_SIZE + 12, 0, BUTTON_SIZE + 12)
    Shadow.Position = UDim2.new(
        INITIAL_POS.X.Scale,
        INITIAL_POS.X.Offset - 6,
        INITIAL_POS.Y.Scale,
        INITIAL_POS.Y.Offset - 6
    )
    Shadow.BackgroundTransparency = 1
    Shadow.Image = "rbxassetid://5554236805"
    Shadow.ImageColor3 = Color3.fromRGB(80, 40, 160)
    ...
    BtnFrame.BackgroundColor3 = Color3.fromRGB(18, 14, 35)
    ...
    -- Imagem do Asset ID (foto do Roblox)
    ...
    -- Botao invisivel para capturar input (ZIndex alto)
    ...
    -- Tooltip pequeno ao lado do botao
    local Tooltip = Instance.new("Frame")
    Tooltip.Size = UDim2.new(0, 95, 0, 24)
    Tooltip.Position = UDim2.new(1, 8, 0.5, -12)
    Tooltip.BackgroundColor3 = Color3.fromRGB(18, 14, 35)
    ...
    TipLabel.Text = "🪐 Saturn Hub"
    TipLabel.TextColor3 = Color3.fromRGB(200, 185, 255)
    ...
    local function setUIState(open)
        ...
        if open then
            ...
            TweenService:Create(Shadow, TweenInfo.new(0.2), {
                ImageColor3 = Color3.fromRGB(110, 60, 220),
                ImageTransparency = 0.4
            }):Play()
        else
            -- Borda apagada, overlay escuro
            TweenService:Create(Stroke, TweenInfo.new(0.2), {
                Color = Color3.fromRGB(50, 35, 90),
                Thickness = 1.5
            }):Play()
            TweenService:Create(Overlay, TweenInfo.new(0.2), {
            ...
            TweenService:Create(Shadow, TweenInfo.new(0.2), {
                ImageColor3 = Color3.fromRGB(30, 20, 60),
    ...
    local function toggleUI()
        setUIState(not isUIOpen)
    end

    -- Clique
    ClickBtn.MouseButton1Click:Connect(toggleUI)

    -- Hover: mostra tooltip + pulsa borda
    ...
    -- ── Arrastar ─────────────────────────────────────────────
    local dragging, dragStart, startBtnPos, startShadowPos = false, nil, nil, nil
...
_G.M3SaveData = _G.M3SaveData or {}
function M3SaveSettings()
    if not writefile then return false end
    pcall(function()
        writefile(_M3FullPath, _M3HttpService:JSONEncode(_G.M3SaveData))
    end)
end
function M3LoadSettings()
    ...
    if ok and res then _G.M3SaveData = res end
end
function M3GetSetting(name, default)
    return _G.M3SaveData[name] ~= nil and _G.M3SaveData[name] or default
end
M3LoadSettings()
...
_G.M3BringRange = _G.M3BringRange or 235
_G.M3MaxBringMobs = _G.M3MaxBringMobs or 3
...
-- AUTO BERRY
_G.M3AutoBerry = false
spawn(function()
    while task.wait(0.1) do
        if _G.M3AutoBerry then
            pcall(function()
...
_G.M3CitizenQuest = false
spawn(function()
    while task.wait(0.1) do
        if _G.M3CitizenQuest then
            pcall(function()
...
_G.M3AutoFishing = false
_G.M3AutoBuyBait = false
_G.M3AutoFishingQuest = false
_G.M3AutoQuestComplete = false
_G.M3AutoSellFish = false
_G.M3SelectedBait = nil
_G.M3SelectedRod = nil
spawn(function()
    ...
    while task.wait(0.5) do
        pcall(function()
            if _G.M3AutoBuyBait and _G.M3SelectedBait then
                local craftRemote = getCraftRemote()
                if craftRemote then craftRemote:InvokeServer("Craft", _G.M3SelectedBait, {}) end
            end
            if _G.M3AutoFishingQuest then
            ...
            if _G.M3AutoQuestComplete then
                local jobsRemote = getJobsRemote()
                if jobsRemote then jobsRemote:InvokeServer("FishingNPC", "FinishQuest") end
            end
            if _G.M3AutoSellFish then
                local jobsRemote = getJobsRemote()
                if jobsRemote then jobsRemote:InvokeServer("FishingNPC", "SellFish") end
            end
            if _G.M3AutoFishing then
                local plr = game.Players.LocalPlayer
                local char = plr.Character or plr.CharacterAdded:Wait()
                local hrp = char:FindFirstChild("HumanoidRootPart")
                if not hrp then return end
                local equippedTool = char:FindFirstChildOfClass("Tool")
                if _G.M3SelectedRod and (not equippedTool or equippedTool.Name ~= _G.M3SelectedRod) then
                    local rodInBag = plr.Backpack:FindFirstChild(_G.M3SelectedRod)
                    if rodInBag then
...
-- STRETCH SCREEN (M3Ow)
_G.M3StretchScreenActive = false
function M3StretchScreen()
    if _G.M3StretchScreenActive then return end
    _G.M3StretchScreenActive = true
    local res = 0.65
    local Camera = workspace.CurrentCamera
    game:GetService("RunService").RenderStepped:Connect(function()
        if _G.M3StretchScreenActive then
            pcall(function()
...
spawn(function()
    ...
    _G.M3FastAttackActive = true
    while task.wait(0) do
        if _G.M3FastAttackActive then
            pcall(function()
                local char = Player.Character
                local hum = char and char:FindFirstChild("Humanoid")
